...
const InventoryAnalysisChart: React.FC<InventoryAnalysisChartProps> =
  React.memo(({ isDark, colors, commonTooltip, commonGrid, data }) => {
    const warehouseStockData = React.useMemo(() => {
      ...
      data.forEach((item) => {
        ...
        grouped[normalizedCategory].sold[item.warehouse_code] =
          item.sold_last_60_days;
        grouped[normalizedCategory].available[item.warehouse_code] =
    ...
    const categories = React.useMemo(
      () => warehouseStockData.map((item) => item.category),
      [warehouseStockData],
    );
    ...
    const option: EChartsOption = React.useMemo(() => {
      ...
      warehouses.forEach((wh, index) => {
        ...
        series.push({
          ...
          type: "bar",
          stack: "Sold",
          data: warehouseStockData.map(
            (item) => (item.sold as Record<string, number>)[wh] || 0,
          ),
          itemStyle: {
            color: colors.palette[index % colors.palette.length],
            borderRadius:
              index === warehouses.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0],
          },
          tooltip: {
            valueFormatter: (value: unknown) =>
              `${String(value)} (Last 60 days sold qty)`,
          },
        });
        // Stack 2: Available
        series.push({
          ...
          stack: "Available",
          data: warehouseStockData.map(
            (item) => (item.available as Record<string, number>)[wh] || 0,
          ),
          itemStyle: {
            color: colors.palette[index % colors.palette.length],
            opacity: 0.5,
          ...
          tooltip: {
            valueFormatter: (value: unknown) =>
              `${String(value)} (Available current day)`,
          },
        });
      });
      ...
      series.push({
        ...
        data: warehouseStockData.map((item) =>
          Object.values(item.available as Record<string, number>).reduce(
            (acc, curr) => acc + curr,
            0,
          ),
        ),
      ...
      series.push({
        ...
        smooth: true,
        data: warehouseStockData.map((item) =>
          Object.values(item.sold as Record<string, number>).reduce(
            (acc, curr) => acc + curr,
            0,
          ),
        ),
        ...
        tooltip: {
          valueFormatter: (value: unknown) =>
            `${String(value)} (Total Sold - Last 60 Days)`,
        },
      });
