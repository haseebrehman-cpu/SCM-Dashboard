...
import { useTopSellingChart, useRegionalSummaryChart, useCategoryDistributionChart } from '../../api/stockReportCharts';
import { Skeleton, Box } from '@mui/material';

const StockReportCharts = () => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';

  const { data: topSellingResponse, isLoading: isTopSellingLoading } = useTopSellingChart();
  const { data: regionalSummaryResponse, isLoading: isRegionalSummaryLoading } = useRegionalSummaryChart();
  const { data: categoryDistributionResponse, isLoading: isCategoryDistributionLoading } = useCategoryDistributionChart();
